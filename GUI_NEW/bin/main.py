...
def selection_window(root):
    ...
    # Body
    body_frame = Frame( root, background=GUI_SETTINGS["c"]["body"]["tabs"]["bg"], height=GUI_SETTINGS["d"]["footer"]["height"], border=0, borderwidth=0,highlightthickness=0)
    body_frame.pack(fill=X)

    # Left
    tab_frame = Frame( body_frame,background=GUI_SETTINGS["c"]["body"]["tabs"]["bg"], width=GUI_SETTINGS["d"]["body"]["tabs"]["width"],height=GUI_SETTINGS["d"]["body"]["tabs"]["height"], border=0, borderwidth=0,highlightthickness=0)
    tab_frame.pack(side=LEFT, expand=False)

    # Right
    working_body_frame = Frame( body_frame, background=GUI_SETTINGS["c"]["body"]["work"]["bg"],width=GUI_SETTINGS["d"]["body"]["work"]["width"],    height=GUI_SETTINGS["d"]["body"]["work"]["height"],border=0, borderwidth=0,highlightthickness=0)
    working_body_frame.pack(side=RIGHT, expand=False)


    # footer
    footer_frame = Frame(  root, background=GUI_SETTINGS["c"]["footer"]["bg"], height=GUI_SETTINGS["d"]["footer"]["height"], border=0, borderwidth=0,highlightthickness=0)
    ...
    for index, each_tab_name in enumerate(app_tab_list):
        app_tab_widges[each_tab_name.lower() + "_frame"] = Frame(tab_frame, background=GUI_SETTINGS["c"]["body"]["tabs"]["bg"], width=GUI_SETTINGS["d"]["body"]["tabs"]["width"], height=80,padx=10,pady=3,border=0, borderwidth=0,highlightthickness=0    )
        app_tab_widges[each_tab_name.lower() + "_frame"].grid(row=index, column=1)

        app_tab_widges[each_tab_name.lower()] = Button(
            app_tab_widges[each_tab_name.lower() + "_frame"], text=each_tab_name, font=GUI_SETTINGS["f"]["body"]["tabs"],  relief=GROOVE, background=GUI_SETTINGS["c"]["body"]["tabs"]["b"]["bg"],  foreground=GUI_SETTINGS["c"]["body"]["tabs"]["b"]["fg"], activebackground=GUI_SETTINGS["c"]["body"]["tabs"]["b"]["abg"],activeforeground=GUI_SETTINGS["c"]["body"]["tabs"]["b"]["afg"],  width=15,        padx=10,
            # border=0, borderwidth=0,highlightthickness=0
        )
        app_tab_widges[each_tab_name.lower()].grid(row=index, column=1)
        app_tab_widges[each_tab_name.lower()][  "command"] = lambda tab_name=each_tab_name.lower(): get_current_selected_tab(tab_name)

    Label(tab_frame, background=GUI_SETTINGS["c"]["body"]["tabs"]["bg"], padx=80, pady=85 +40).grid(row=index + 1, column=1)
    ...
    ok_BTN = Button( footer_right_frame, text="Ok", width=15,  height=10, font=GUI_SETTINGS["f"]["footer"],background="#24b982", foreground="#125d41", activebackground='#20a775', activeforeground='#166f4e', border=0, borderwidth=0,highlightthickness=0, command= making_run_command)
